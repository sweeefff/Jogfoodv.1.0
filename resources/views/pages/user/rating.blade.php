...
@section('content')
    <div class="min-h-screen bg-amber-100 flex items-center justify-center p-4">
        <div
            class="max-w-md w-full bg-white rounded-2xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-xl">
            <div class="p-6">
                ...
                <div class="flex items-center space-x-4 p-4 bg-orange-50 rounded-xl mb-6">
                    <img src="{{ asset('assets/img/menu/' . $menu->gambar_menu) }}"
                        class="w-16 h-16 object-cover rounded-lg border-2 border-orange-200" alt="{{ $menu->nama }}">
                    <div>
                    ...
                    <form action="{{ route('rating.store', [$menu->id_menu, $id_detail]) }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="star-rating flex flex-row-reverse justify-center text-3xl">
                                @for($i = 5; $i >= 1; $i--)
                                    ...
                                    <label for="star{{ $i }}"
                                        class="cursor-pointer text-gray-300 hover:text-amber-400">&#9733;</label>
                                @endfor
                            </div>
                            @error('rating')
                        ...
                        <div>
                            ...
                            <textarea name="review" rows="3"
                                class="w-full border border-amber-300 rounded-lg p-2 focus:ring-amber-400 focus:border-amber-400"
                        ...
                        <button type="submit"
                            class="w-full bg-amber-500 hover:bg-amber-600 text-white font-semibold py-3 rounded-lg shadow transition duration-200">
                            Kirim Rating
                        </button>
                    </form>
                @endif
                <div class="mt-6 text-center">
                    <a href="{{ route('detail', $menu->id_menu) }}" class="text-amber-600 hover:underline text-sm">Kembali
                        ke Detail Produk</a>
                </div>
            </div>
        </div>
